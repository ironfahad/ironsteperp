...
function get_open_balance($supplier_id, $to)
{
    ...
    $sql .= "SUM(IF(t.type = ".ST_SUPPINVOICE.", 1, -1) *
        ...
        WHERE t.supplier_id = ".db_escape($supplier_id)." AND t.type NOT IN (0,21)";
    if ($to)
...
//////////////////
function get_open_balance_cre($supplier_id, $to)
{
    if ($to)
        $to = date2sql($to);
    $sql .= "SELECT SUM(abs(t.ov_amount + t.ov_gst + t.ov_discount)) 
        FROM ".TB_PREF."supp_trans t
        WHERE t.supplier_id = ".db_escape($supplier_id)." AND t.type =21";
    if ($to)
        $sql .= " AND t.tran_date < '$to'";
    $sql .= " GROUP BY supplier_id";
...
function get_open_balance_jv($supplier_id, $to)
{
    if ($to)
        $to = date2sql($to);
    $sql .= "SELECT SUM((t.ov_amount + t.ov_gst + t.ov_discount)) 
        FROM ".TB_PREF."supp_trans t
        WHERE t.supplier_id = ".db_escape($supplier_id)." AND t.type =0";
    if ($to)
        $sql .= " AND t.tran_date < '$to'";
    $sql .= " GROUP BY supplier_id";
...
function getTransactions2($supplier_id, $from, $to, $transno,$type)
{
	$from = date2sql($from);
	$to = date2sql($to);
...
if($supp_ledger==1)
	{
$headers = array(_('Date'), _('Naration'), _('Quantity'), _('Rate'), _('Charge'),
			_('Credits'),_('Adjustment'),_('Balance'));
	}
	else
	{
$headers = array(_('Date'), _('Naration'), _(''), _(''), _('Charges'),
			_('Credits'),_('Adjustment'),_('Balance'));
}
	if ($show_balance)
		$headers[7] = _('Balance');
	...
	while ($myrow=db_fetch($result))
	{
		...
		$bal_credit=get_open_balance_cre($myrow['supplier_id'], $from);
		$bal_jv=get_open_balance_jv($myrow['supplier_id'], $from);
		$init[0] = $init[1] = 0.0;
		...
		if ($show_balance)
		{
			$init[3] = $init[0] - $init[1]-$bal_credit;
			$accumulate += $init[3];
		}	
		else	
		...
		$rep->AmountCol(6, 7, ($bal_credit+abs($bal_jv)), $dec);
		$rep->AmountCol(7, 8, $init[0]-$init[1]-$bal_credit+$bal_jv, $dec);
		$open_bal=$init[0]-$init[1]-$bal_credit+$bal_jv;
		$total = array(0,0,0,0);
		for ($i = 0; $i < 4; $i++)
		{
			$total[$i] += $init[$i];
		...
		while ($trans=db_fetch($res))
		{
			...
			$rep->NewLine(1, 2);
			$rep->TextCol(1, 4, $systypes_array[$trans['type']]." ".$trans['trans_no']);
			...
			if ($trans['TotalAmount'] > 0.0)
			{
			    ...
			    if($trans['type'] == 21 || $trans['type'] == ST_JOURNAL)
			    {
			...
			if ($show_balance)	
				...
				$rep->AmountCol(7, 8, $accumulate+$open_bal, $dec);
				...
				$res2 = getTransactions2($myrow['supplier_id'], $from, $to, $trans['trans_no'],$trans['type']);
				$rep->NewLine();
		...
		{//display_error($jv);
		$charges +=$total[0]-$jv;
		$credit +=$total[1]-$credit_note;
		$adjustment +=$credit_note+$jv+$bal_credit+abs($bal_jv);
		$balance +=$accumulate+$open_bal;
			$rep->AmountCol( 4,  5, $total[0]-$jv, $dec);
			$rep->AmountCol( 5,  6, $total[1]-$credit_note, $dec);
			
			$rep->AmountCol( 6,  7, $credit_note+$jv+$bal_credit+abs($bal_jv), $dec);
			$rep->AmountCol( 7,  8, $accumulate+$open_bal, $dec);
			$jv=$credit_note=$bal_credit=$bal_jv=$open_bal=0;
			$total[$i] = 0.0;
		}
    	$rep->Line($rep->row  - 4);
    	$rep->NewLine(2);
...
//	for ($i = 0; $i < 4; $i++)
			...
			$rep->AmountCol( 5,  6, $credit, $dec);
			$rep->AmountCol( 6,  7, $adjustment, $dec);
			$rep->AmountCol( 7,  8, $balance, $dec);
	$rep->Line($rep->row  - 4);
	$rep->NewLine();
    $rep->End();
}
