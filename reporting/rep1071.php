...
function print_invoices()
{
   ...
   // $cols = array(8, 30, 65, 90, 120, 145, 400);
    ...
    if ($email == 0)
    {
        $rep = new FrontReport(_('ST_SALESINVOICE'), "InvoiceBulk", 'POS3', '9');
        $rep->SetHeaderType('Header10');
        $rep->currency = $cur;
        $rep->Font();
        $rep->Info($params, $cols, $headers2, $aligns);
    }
 ...
 $rep->Info($params, $cols, $headers, $aligns);
        ...
        while ($myrow2=db_fetch($result))
        {
            ...
            $line_disc_name =0;// get_discount_name($myrow2['text1']);
            $rep->NewLine();
...
//        $rep->NewLine();
        $total_disc_name =0;// get_discount_name($sales_order["h_text3"]);
        ...
        // // $TotalName = 'After Discount';
        //     $rep->NewLine();
        //     $rep->TextCol(0, 3,  'After Discount', - 2);
...
$rep->Font('');
       ...
       // $rep->Font('bold');
        ...
        if( $rep->company['domicile']!=''){
            
      //  $rep->TextCol(0, 2, "Wifi:", -2);
        $rep->TextCol(0, 6, "Location: ".$rep->company['domicile'] . "           ", -2);
        
        }
       // $user = get_user_id($i,ST_SALESINVOICE);
        //  $rep->NewLine();
        //$rep->TextCol(0, 6, "User:                                       ".get_user_name_70123($user), - 2);
        $rep->NewLine();
