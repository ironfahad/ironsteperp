...
function get_item_last_price($customer_id)
{
	$sql = "SELECT labour_cost FROM ".TB_PREF."stock_master WHERE stock_id=".db_escape($customer_id);
...
{
    //qty_row(_(":"), 'quantity', null, null, null, $dec);
	ref_cells(_("Quantity:"), 'quantity', '',null, '', true);
//	display_error($_POST['quantity']);
    date_row(_("Date") . ":", 'date_', '', true);
	hidden('RequDate', '');
...
$total =round(get_item_last_price(get_post('stock_id'))*$_POST['quantity']);
//list_updated('Labour');
$Ajax->activate('Labour');
//display_error($total);
//	amount_row($wo_cost_types[WO_LABOUR], 'Labour');
	text_row($wo_cost_types[WO_LABOUR],'Labour', $total, 10, 10);
